...
class Blockchain {
    ...
    _addBlock(block) {
        let self = this;
        let errorLog = [];
        //let time = new BlockClass.Block();
        return new Promise(async (resolve, reject) => {
          let blockHeight = await this.getChainHeight();
          let prevBlock = await this.getBlockByHeight(blockHeight);
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise(async(resolve, reject) => {
            ...
            for(let block of self.chain){
                if(block.height !== 0){
                let ownerData = await block.getBData();
                    if(ownerData.address === address){
                        stars.push(ownerData);
    ...
    validateChain() {
        let self = this;
        let errorLog = [];
        let validBlock = self.chain;
        return new Promise(async (resolve) => {
            //Loop throw all blocks in the chain
            for(let block of validBlock){
